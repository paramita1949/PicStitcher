...
class ImageProcessorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("图片批量拼接与反色处理工具V1.2")
    ...
    def create_widgets(self):
        ...
        # RGB颜色设置区域
        rgb_frame = ttk.LabelFrame(main_frame, text="反色颜色设置 (RGB/16进制互转)")
        rgb_frame.pack(fill=tk.X, pady=10)

        # 16进制输入框区域
        hex_input_frame = ttk.Frame(rgb_frame)
        hex_input_frame.pack(fill=tk.X, padx=10, pady=5)

        ttk.Label(hex_input_frame, text="16进制:").pack(side=tk.LEFT)
        self.hex_var = tk.StringVar(value=f"#{self.yellow_text_r:02x}{self.yellow_text_g:02x}{self.yellow_text_b:02x}")
        self.hex_entry = ttk.Entry(hex_input_frame, textvariable=self.hex_var, width=10)
        self.hex_entry.pack(side=tk.LEFT, padx=5)
        self.hex_var.trace_add('write', self.on_hex_change)

        # RGB输入框区域
        rgb_input_frame = ttk.Frame(rgb_frame)
        rgb_input_frame.pack(fill=tk.X, padx=10, pady=5)

        # R值输入
        ttk.Label(rgb_input_frame, text="R:").pack(side=tk.LEFT)
        ...
        # G值输入
        ttk.Label(rgb_input_frame, text="G:").pack(side=tk.LEFT, padx=(10,0))
        ...
        # B值输入
        ttk.Label(rgb_input_frame, text="B:").pack(side=tk.LEFT, padx=(10,0))
        ...
        # 颜色预览
        self.color_preview = tk.Label(rgb_input_frame, text="  预览  ",
